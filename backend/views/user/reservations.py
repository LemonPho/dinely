...
def _find_reservation_by_code_or_email(code, email):
    """
    Helper function to find reservation by code or email.
    Returns (reservation, error_dict) tuple.
    """
    if not code and not email:
        return None, {"error": "Either code or email is required"}
...
def get_user_reservation(request):
    ...
    if not code and not email:
        return JsonResponse({"error": "Either code or email is required"}, status=400)
...
def edit_user_reservation(request):
    ...
    # Get code or email from request body
    code = data.get("code", None)
    email = data.get("email", None)

    # Find reservation
    reservation, error = _find_reservation_by_code_or_email(code, email)
    if error:
