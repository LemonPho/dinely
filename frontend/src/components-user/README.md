...
return (
    <input type={"email"} className={"mb-2"} placeholder={"Email"} value={registerInput.email} onChange={handleEmailInput} />
)
```
...
### Estructura general
- function Nombre*Uso*(): El nombrado de la funcion de React se deberia hacer con este estilo: el nombre de su esencia digamos, el nombre de la pagina podria ser o su uso, enseguida iria su utilidad, esto podria ser una pagina (Page), un componente de una pagina (Component), etc. Lo importante es seguir un patron.
