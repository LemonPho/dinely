...
### Estructura general
- export ... = createContext(): Esta linea se pone para initializar el objeto del contexto, la nombre del objeto deberia ir como: el contexto que deseas compartir, y enseguida *Context*. Un ejemplo es el `userContext`, queremos compartir el usuario y le sigue *Context*.
...
```javascript
const { variable, funcion } = useNombreContext();
```
...
```
En este caso el `{children}` de lo cual se hablo anteriormente seria el componente `ManageTables` y sus hijos, y unicamente esos podrian usar el `ManageTablesContext`. Esta informacion se colocaria en algun `return ()` de algun componente de `React`.